...
function bounding_boxes = anomalyDetection(rec_hs, numb_pca_components, num_ica_components, ICA_max_iterations, similarity_threshold, opts_lambda, opts_max_iter, lambda, overlap_threshold, min_area, max_area, min_anomalous_pixels)
    ...
    save_dir = fullfile(pwd, 'anomaly_visuals');
    ...
    imwrite(uint8(similarity_mask * 255), fullfile(save_dir, 'similarity_mask.png'));
    ...
    if ndims(DataTest) == 2
        DataTest = reshape(DataTest, H, W, 1);   % H × W × 1
    end
    ...
    % --- Save refined TLRSR anomaly map ---
    imwrite(uint8(anomaly_map_pred * 255), fullfile(save_dir, 'refined_roi.png'));
